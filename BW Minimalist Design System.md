...
Across `/men`, the trail-running listing, the trail shoe PDP, `/membership`, and performance lifestyle editorial, the same chrome appears in identical proportions — only the photography and copy change. That is the system's signature: maximum editorial expression in the imagery, maximum mechanical restraint everywhere else.

**Key Characteristics:**
- Editorial campaign hero with `{typography.display-campaign}` (Bebas Neue, 96px, line-height 0.9, uppercase) burned directly into full-bleed photography
...
> **Reference surfaces:** `/shop` (primary), `/collections/trail-running`, `/product/trail-shoe`, `/membership`, `/collections/performance-lifestyle`. The chrome palette is identical across all five — only photography varies.

### Brand & Accent
- **Core Black** (`{colors.ink}` — `#111111`): The brand's only "color." It is the primary CTA, the swatch dot, the active filter chip, the campaign overlay, the headline color, and the body text. When the system needs to assert anything, it goes black.
...
- **Accent Pink** (`{colors.accent-pink}` — `#ed1aa0`): Limited collaboration / women's collection moments.
...
- **Accent Teal** (`{colors.accent-teal}` — `#0a7281`): Trail / outdoor / outdoor editorial accent in lockups.
- **Accent Pink Deep** (`{colors.accent-pink-deep}` — `#4c012d`): Deepest editorial overlay tint, used as wash on heritage / heritage tiles.

## Typography

### Font Family
- **Bebas Neue** (display campaign only) — open-source geometric condensed sans for the towering uppercase headlines burned into campaign hero photography. Falls back to Inter → Arial → sans-serif.
- **Inter** (headings 16–32px) — modern Inter cut tuned for display sizes; carries every section title, PDP product name, and dialog headline.
- **Inter** (UI 12–16px) — buttons, captions, swatch labels, badge text. The system's UI workhorse.
- **Inter** (body and links) — long-form body and underlined inline links.
...
The system's open-source type pairing uses **Inter** (Display 700 for body chrome, Display 500 for buttons) with **Bebas Neue** or **Anton** at 96px/0.9 line-height for the campaign headline tier. Tighten letter-spacing slightly (-0.5%) on the alternate display face to preserve the intended optical weight.
...
### Principles
The system runs on extreme typographic contrast: a single 96px uppercase display tier reserved for editorial campaign moments, and a quiet 12–16px Inter/Medium tier carrying everything else. There is almost no middle ground — the jump from `{typography.heading-xl}` (32px) directly to `{typography.body-strong}` (16px) is intentional and creates the "billboard above, catalog below" effect across every page. Letter-spacing is left at 0 (Bebas Neue and Inter are both cut for tight optical fit at scale).

### Note on Font Substitutes
The closest open-source path to the system's display tier is **Bebas Neue** (free, geometric condensed) at 96px / 0.9 / uppercase / 500. For UI text, **Inter** is the safest substitute — match weights 400/500 and the system reads almost identically at button and caption sizes.
...
### Decorative Depth
Depth in the system's system comes entirely from photography, not from CSS effects:
- **Editorial campaign tiles** create depth via cinematic perspective — a runner on a trail, a model in a courtyard — with the condensed display headline overlaid in white or `{colors.ink}` directly on the image.
...
- **Editorial campaign hero:** ~16:9 or wider cinematic crop, full-bleed across the content max-width, with the condensed display headline burned into the lower-left or upper-left third.
...
**`button-primary`** — the universal primary CTA
...
- Pressed state lives in `button-primary-active` — the bg stays `{colors.ink}` while the surface shrinks to `scale(0.5)` with `opacity: 0.5` (the system's signature "tap collapse" feedback that's extracted across all five pages).
...
- Layout: BW monogram logo at left (32×32), centered nav row ("New & Featured · Men · Women · Kids · Collections · Collaborations · Sport"), right cluster (search pill, wishlist heart icon, bag icon).
...
**Top Nav (Mobile)**
- Hamburger menu icon (left), BW monogram (center), search + bag icons (right).
...
### Collapsing Strategy
- **Primary nav:** desktop center cluster → mobile drawer triggered by hamburger at left of the BW monogram.
...
- **Hover states not documented** by system policy — the source implementation uses element-hover and text-hover tokens, but these are not included here.
